...
public class ProGifRecorderComponent : MonoBehaviour
{
	...
	private void _Setup(bool autoAspect, int width, int height, int fps, float recorderTime, int repeat, int quality, Vector2 gifAspectRatio)
	{
        ...
        if (m_AutoAspect)
        {
            float deviceAspect = (Camera.main != null) ? Camera.main.aspect : Screen.width / Screen.height;
            if (deviceAspect < 1f)
            {
                m_Height = (int)Mathf.Clamp(Mathf.RoundToInt(m_Width / deviceAspect), 8, Mathf.Infinity);
            }
            else
            {
                m_Width = (int)Mathf.Clamp(Mathf.RoundToInt(m_Width * deviceAspect), 8, Mathf.Infinity);
                m_Height = (int)Mathf.Clamp(Mathf.RoundToInt(m_Width / deviceAspect), 8, Mathf.Infinity);
            }
        }
        else
