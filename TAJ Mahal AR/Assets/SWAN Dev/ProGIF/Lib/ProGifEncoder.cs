...
internal class ProGifEncoder
{
	...
	public void Finish()
	{
		...
		m_OnEncodingComplete?.Invoke(m_EncoderIndex);
	}
...
//		m_PixelsAlpha = new Byte[W * H];
	...
	protected void AnalyzePixels()
	{
		...
		ProGifNeuQuant nq = new ProGifNeuQuant(m_Pixels, len, m_SampleInterval, (m_AutoTransparent && m_HasTransparentPixel));
        ...
        for (int i = 0; i < nPix; i++)
		{
			...
			if(m_TransparentFlag == 1)
			{
				...
				else if(!m_AutoTransparent && m_Pixels[k-3] == tR && m_Pixels[k-2] == tG && m_Pixels[k-1] == tB)
				{
					index = m_TransparentColorIndex;
				}
			}
	...
	protected void SetTransparencyIndexAndFlag(Color32 c32)
	{
		...
		//	+ " | m_AutoTransparency: " + m_AutoTransparent + " | m_HasTransparentPixel: " + m_HasTransparentPixel
