...
public class ProGifDecoder
{
    ...
    private IEnumerator DecodeTextureCoroutine(GifData gifData, Action<List<GifTexture>> callback, Action<GifTexture> onFrameReady = null)
    {
        ...
        //Color32 lastBgColor = new Color32(0, 0, 0, 0);
        ...
        for (int i = 0; i < gifData.m_imageBlockList.Count; i++)
        {
            ...
            if (doDecode)
            {
                ...
                Color32[] colors = null;
                //colors = SetPixel(gifData, gifTexList, decodeIndex, disposalMethodList, i, decodedData, colorTable, (i > 0 ? lastBgColor : bgColor), transparentIndex);
    ...
    private bool SetGifBlock(byte[] gifBytes, ref int byteIndex, ref GifData gifData)
    {
        ...
        {
            Debug.LogError("(SetGifBlock) " + ex.Message + "\n byteIndex: " + byteIndex);
            return false;
        }
    ...
    /// </summary>
    private void SetPixelRow(ref Color32[] pixels, int texWidth, int texHeight, int y, ProGifDecoder.ImageBlock imgBlock, byte[] decodedData,
